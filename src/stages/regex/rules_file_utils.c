...
static int
rules_file_utils.hs_flags_to_rxp(char *output_flags, char *input_flags, int *skipped_flags)
{
	size_t i;
	int j;
...
rules_file_utils_write_rxp(FILE *file, char *exp, char *id, char *flags, int *skipped_flags)
{
	...
	ret = rules_file_utils.hs_flags_to_rxp(&rxp_flags[0], flags, skipped_flags);
	if (ret) {
